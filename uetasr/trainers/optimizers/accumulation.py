...
class GradientAccumulator(tf.keras.optimizers.Optimizer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            inner_optimizer: str or `tf.keras.optimizers.Optimizer` that will
                be used to compute and apply gradients.
            ...
            for grad, var in grads_and_vars:
                ...
                if isinstance(grad, tf.IndexedSlices):
                    ...
                    def _get_grad():
                        ...
                        indices = tf.squeeze(
                            tf.where(
                                tf.reduce_sum(
                                    new_grad,
                                    axis=list(range(len(new_grad.shape))[1:])
                                ) != 0
                            ), axis=-1
                        )
                    ...
                    def _get_grad():
                        new_grad = handle.read_value()
                        if self._reduction == "MEAN":
                            new_grad /= tf.cast(self._accum_steps,
                                                new_grad.dtype)
                        handle.assign(
                            tf.zeros_like(handle),
                            use_locking=self._use_locking,
    ...
    def step(self, variable):
        if self._step is not None:
            raise RuntimeError("Cannot set `step` to a new Variable after "
